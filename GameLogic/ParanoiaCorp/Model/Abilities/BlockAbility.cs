...
namespace GameLogic.ParanoiaCorp.Model.Abilities
{
    ...
    public class BlockAbility : IAbility
    {
        public ITarget[] Targets { get; }
        public IExecutor Holder { get; }

        public BlockAbility(IExecutor holder, ITarget target)
        {
            Holder = holder;
            Targets = new[] { target };
        }
        ...
        {
            bool weBlocked = order.Exists(a => a is BlockAbility && a.Targets[0] == Holder);
            if(weBlocked)
            {
                //If we got blocked by other blocker, we are not prioritized
                order.Remove(this);
                return;
            }
