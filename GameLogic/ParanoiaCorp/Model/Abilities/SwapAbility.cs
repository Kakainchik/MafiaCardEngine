...
namespace GameLogic.ParanoiaCorp.Model.Abilities
{
    ...
    public class SwapAbility : IAbility
    {
        ...
        public SwapAbility(IExecutor holder, ITarget target1, ITarget target2)
        {
            Holder = holder;
        ...
        {
            //Find all actions where their target is swapper primary one
            List<IAbility> executors1to2 = order.FindAll(a =>
            {
                return a != this && a.Targets.Any(t => t == Targets[0]);
            });
            //Find all actions where their target is swapper secondary one
        ...
        {
            SwapVisitor visitor = new SwapVisitor(Holder);
