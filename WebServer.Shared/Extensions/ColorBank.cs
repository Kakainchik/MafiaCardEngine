...
namespace WebServer.Shared.Extensions
{
    public static class ColorBank
    {
        ...
        internal readonly static RGB Gainsboro = new(0xDC, 0xDC, 0xDC);
        ...
        internal readonly static RGB Peru = new(0xCD, 0x8, 0x3F);
        internal readonly static RGB Plum = new(0xDD, 0xA0, 0xDD);
        internal readonly static RGB SaddleBrown = new(0x8B, 0x45, 0x13);
        ...
        public static RGB[] GetRandomColors(int length)
        {
            if(length <= 0 || length > 50)
            {
                throw new ArgumentOutOfRangeException(nameof(length), "Cannot return 0 or 50+ colors.");
            }

            RGB[] result = new RGB[length];
            for(int i = 0; i < length; i++)
            {
                result[i] = (RGB)fi[i].GetValue(null)!;
            }
